...
# Let's cleanly replace the entire bad python block with a perfectly aligned one.
bad_block_regex = r"        - name: .* Patch MapboxMaps Concurrency\s*run: \|\s*python3 - <<'PY'.*?            PY"
...
text = re.sub(bad_block_regex, lambda m: perfect_block, text, flags=re.DOTALL)

    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: DefaultTextStyle(
        style: const TextStyle(
          color: Colors.white,
          decoration: TextDecoration.none, // Removes all yellow lines
        ),
...
class _TripDetailScreenState extends State<TripDetailScreen>
  ...
  Color _speedColor(double speedKmh) {
    if (!speedKmh.isFinite || speedKmh <= 0.0) return _kBlue;
    if (speedKmh < 15) return _kBlue.withValues(alpha: 0.8);
    if (speedKmh < 40) return _kBlue;
    if (speedKmh < 70) return Colors.white;
    if (speedKmh < 100) return _kCyan;
    return _kRed;
  }
  ...
  void initState() {
    ...
    if (_hasRoute) {
      ...
      _hudSpeed.value = _replaySpeedMph * 1.60934;
    }
  ...
  void _animatedMove(LatLng destination, double zoom, {bool force = false}) {
    ...
    _moveAnimController?.stop();
    if (oldAnim != null) _moveAnim = null;
    _moveAnimController?.dispose();
    ...
    try {
      ...
      try {
        if (_mapReady && mounted) _mapController.move(destination, safeZoom);
      } catch (_) {}
      return;
    }
    ...
    final Animation<LatLng> anim =
      ...
      CurvedAnimation(parent: ctrl, curve: Curves.easeInOutCubic),
    );
    _moveAnim = anim;
  ...
  void _skipReplay(int seconds) {
    ...
    final int newSeconds =
        (current.inSeconds + seconds).clamp(0, totalDur.inSeconds);
  ...
  void _onReplayTick() {
    ...
    final double t = newProgress.clamp(0.0, 1.0);
    ...
    final double endSpd =
        _route[(_replayIndex + 1).clamp(0, _route.length - 1)].speedMph;
    ...
    _hudSpeed.value = newSpeed * 1.60934;
  ...
  void _scrubReplay(double value) {
    ...
    final double progress = (scaled - index).clamp(0.0, 1.0);
  ...
  Widget _buildSpeedGradientLayer() {
    ...
    for (int i = 1; i < _route.length; i++) {
      ...
      final double avgKmh = avgMph * 1.60934;
      final Color color = _speedColor(avgKmh);
  ...
  Widget _buildSpeedHud(SettingsService settings) {
    return ValueListenableBuilder<double>(
      valueListenable: _hudSpeed,
      builder: (_, double speedKmh, __) {
        final double displaySpeed = settings.toDisplaySpeed(speedKmh / 1.60934);
        final Color accent = _speedColor(speedKmh);
          ...
          child: SafeArea(
            ...
            child: Container(
              constraints: BoxConstraints(
                maxHeight: MediaQuery.sizeOf(popupContext).height * 0.82,
              ),
              decoration: const BoxDecoration(
                color: _kSurface,
...
class _TripExportSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final EdgeInsets padding = MediaQuery.viewPaddingOf(context);
    final double bottomPad = math.max(padding.bottom, 14.0);
    final double distance = settings.toDisplayDistance(trip.distanceMiles);
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: ConstrainedBox(
              ...
              child: Container(
                padding: EdgeInsets.fromLTRB(18, 14, 18, bottomPad + 14),
                decoration: BoxDecoration(
...
class _SafeText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      ...
      softWrap: softWrap,
      textAlign: textAlign,
      // Forcing decoration none again explicitly just in case standard overrides miss
